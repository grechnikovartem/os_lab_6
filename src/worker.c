...
void function(void *worker2, void *worker3, char *msg, int key, int global_key, int port2, int port3)
{
    ...
    if (key < global_key)
        zmq_msg_send(&req, worker2, 0);
    else
        zmq_msg_send(&req, worker3, 0);
    zmq_msg_close(&req);
    zmq_msg_t reply;
    zmq_msg_init(&reply);
    if (key < global_key)
        zmq_msg_recv(&reply, worker2, 0);
    else
        zmq_msg_recv(&reply, worker3, 0);
    zmq_msg_close(&reply);
}
        ...
        if (!strcmp(md->message, "create\0"))
        {
            if (n2 == 0 && md->key < key)
            {
                ...
                if (fork() == 0)
                {
                    ...
                    function(worker2, worker3, "create\0", md->key, key, md->port2, md->port3);
            }
